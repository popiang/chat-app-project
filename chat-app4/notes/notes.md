...
13. create db.js in libs, import mongoose and create connectDB async function
...
22. now we want to add updateProfile route
	...
	- continue with updateProfile
	- try catch
	- get profilePic from req.body
	  - edit profile only allow updating profile pic
	- get userId from req.user._id
	  - we get this from protectRoute coz we assign user to req.user
	- if no profilePic, return 400 with message
	- if ok, upload the file to cloudinary and get the response
	- next, update data in table
	  - user findUserByIdAndUpdate
	- return 200 with message
	- in error, console log and return 500 with message
...
29. now to send message
	...
	- create sendMessage controller
	  ...
	  - if there's image, upload to cloudinary, get the response, and get the imageUrl and assign to imageUrl var
...
30. now go to then Navbar and start creating the complete header, simply follow the code from video, too long and too many class names
